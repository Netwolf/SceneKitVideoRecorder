...
public class SceneKitVideoRecorder: NSObject, AVAudioRecorderDelegate {
    ...
    public func setupAudio() {
        ...
        do {
            try recordingSession.setCategory(AVAudioSessionCategoryPlayAndRecord, with: [.mixWithOthers, .defaultToSpeaker, .allowAirPlay])
    ...
    private func mergeVideoAndAudio(videoUrl:URL, audioUrl:URL) -> Future<Void, NSError>
    {
        ...
        udta.value = Data(bytes: [0, 0, 0, 0] as [UInt8]) as NSCopying & NSObjectProtocol
        ...
        grey.value = Data(bytes: [0, 0, 0, 0] as [UInt8]) as NSCopying & NSObjectProtocol
